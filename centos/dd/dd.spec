...
%define logsdir %{buildroot}/var/log/%{name}
...
install -D ../centos/dd/%{name}_mod_wsgi_daemons.conf %{buildroot}/etc/httpd/conf.d/%{name}_mod_wsgi_daemons.conf
install -D ../centos/dd/%{name}_mod_wsgi.conf %{buildroot}/etc/httpd/conf.d/%{name}_mod_wsgi.conf
install -D ../centos/dd/django.wsgi %{buildinstalldir}/django.wsgi
install -m 0755 -D ../centos/dd/%{name}-manage.py %{buildroot}/%{_bindir}/registry%{name}
