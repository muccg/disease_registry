...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for diagnosis in orm['sma.Diagnosis'].objects.all():
            diagnosis.id = diagnosis.patient_id
            diagnosis.save()

        for respiratory in orm['sma.Respiratory'].objects.all():
            respiratory.id = respiratory.diagnosis_id
            respiratory.save()
        ...
        for motorfunction in orm['sma.MotorFunction'].objects.all():
            motorfunction.id = motorfunction.diagnosis_id
            motorfunction.save()

        for surgery in orm['sma.Surgery'].objects.all():
